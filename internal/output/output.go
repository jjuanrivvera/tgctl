// Package output renders Bot API JSON results in table/json/yaml/csv. One renderer serves
...
// preferredOrder lists Bot API fields that should lead a table when present, before the rest
// fall back to alphabetical. Deterministic ordering is required by GOAL.md §11.
var preferredOrder = []string{
	"update_id", "message_id", "id", "command", "type", "status",
	"chat", "chat.id", "chat.title", "chat.type", "from", "from.id", "from.username",
	"user", "user.id", "user.username", "username", "first_name", "last_name", "title",
	"text", "description", "date", "url",
}
