// Package config resolves tgctl configuration with a manual flag > env > file > default
// precedence (no Viper, per the cliwright house pattern). Profiles let one user drive several
// bots; the secret token never lives here — only in the OS keyring (see internal/auth).
package config
...
// Profile holds the non-secret settings for one bot.
type Profile struct {
	BaseURL    string `yaml:"base_url,omitempty"`
	AuthMethod string `yaml:"auth_method,omitempty"`
	BotID      string `yaml:"bot_id,omitempty"` // the token's numeric prefix; not secret
}
...
type Config struct {
	...
	Aliases        map[string]string  `yaml:"aliases,omitempty"` // name -> expansion (e.g. "ping" -> "bot info")

	path string `yaml:"-"`
}

// Dir returns the configuration directory: $XDG_CONFIG_HOME/tgctl if set, else ~/.tgctl-cli.
func Dir() (string, error) {
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "tgctl"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".tgctl-cli"), nil
}
...
// across tgctl instead of a config framework.
...
// non-loopback host, so a bot token is never sent in the clear over the network.
func ValidateBaseURL(raw string) error {
	...
	if u.Scheme == "http" && !isLoopback(u.Hostname()) {
		return fmt.Errorf("refusing cleartext http:// for non-loopback host %q (the bot token would leak); use https", u.Hostname())
	}
	return nil
}
