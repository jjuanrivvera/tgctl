## tgctl chat set-photo

Set a chat's photo

```
tgctl chat set-photo [flags]
```

### Examples

```
  tgctl chat set-photo --chat @group --photo ./logo.png
```
...
```
  ...
  -h, --help           help for set-photo
      --photo string   local image path (URLs/file_id not accepted by Telegram here)
```
